...
class BrandstoreScraper(BaseScraper):
    ...
    def scrape(self) -> Iterator[ProductRow]:
        ...
        if not working_url:
            logger.warning("[brandstore] could not find working category URL")
            return
        ...
        for page in range(1, 20):
            ...
            try:
                ...
                cards = soup.select(
                    ".product-card, .catalog-item, .product-item, [class*='product-card']"
                )
                if not cards:
                    logger.info("[brandstore] page %d: no cards, stopping", page)
                    break

...
class IdeaScraper(BaseScraper):
    ...
    def scrape(self) -> Iterator[ProductRow]:
        for slug in ("product/category/smartfonlar", "catalog/smartfony", "smartphones"):
            page = 1
            found_any = False
            while page <= 15:
                try:
                    ...
                    cards = soup.select(
                        ...
                        ".item-product, li.product, .product_list_item"
                    )
                    if not cards:
                        break
                    ...
                    logger.warning(f"[idea] /{slug} page {page} error: {exc}")
                    break
                page += 1
            if found_any:
                break

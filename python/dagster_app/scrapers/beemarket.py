...
class BeemarketScraper(BaseScraper):
    ...
    def scrape(self) -> Iterator[ProductRow]:
        for slug in ("catalog/smartfony", "smartphones", "catalog/telefony", "smartfonlar"):
            page = 1
            found_any = False
            while page <= 15:
                try:
                    ...
                    cards = soup.select(
                        ...
                        "[class*='item-product'], .bee-product, li.product"
                    )
                    if not cards:
                        break
                    ...
                    for card in cards:
                        ...
                        title_el = card.select_one(
                            "a[class*='name'], .product-name, h3 a, h2 a, .item-name a"
                        )
                        if not title_el:
                            continue
                    ...
                    logger.warning(f"[beemarket] /{slug} page {page} error: {exc}")
                    break
                page += 1
            if found_any:
                break

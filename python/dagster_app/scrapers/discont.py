...
class DiscontScraper(BaseScraper):
    ...
    def scrape(self) -> Iterator[ProductRow]:
        for slug in ("catalog/smartfony", "smartfonlar", "catalog/telefony", "smartphones"):
            page = 1
            found_any = False
            while page <= 15:
                try:
                    ...
                    for card in cards:
                        ...
                        title_el = card.select_one(
                            "a[class*='name'], .product-name, h3 a, h2 a"
                        )
                        if not title_el:
                            continue
                    ...
                    logger.warning(f"[discont] /{slug} page {page} error: {exc}")
                    break
                page += 1
            if found_any:
                break

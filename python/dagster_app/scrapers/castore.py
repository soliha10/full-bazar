...
class CastoreScraper(BaseScraper):
    ...
    def scrape(self) -> Iterator[ProductRow]:
        for slug in ("phones", "iphone", "smartphones", "catalog/smartfony", "catalog/iphone"):
            page = 1
            found_any = False
            while page <= 10:
                try:
                    ...
                    cards = soup.select(
                        ...
                        "[class*='item-product'], li.product, .store-item"
                    )
                    if not cards:
                        break
                    ...
                    logger.warning(f"[castore] /{slug} page {page} error: {exc}")
                    break
                page += 1
            if found_any:
                break

...
export function Checkout({ cartItems }: CheckoutProps) {
  ...
  const topFields = [
    { label: t.checkout.firstName,    name: 'firstName', type: 'text' },
    { label: t.checkout.lastName,     name: 'lastName',  type: 'text' },
    { label: t.checkout.emailAddress, name: 'email',     type: 'email' },
    { label: t.checkout.phoneNumber,  name: 'phone',     type: 'tel' },
  ];

  const cityFields = [
    { label: t.checkout.city,     name: 'city' },
    { label: t.checkout.state,    name: 'state' },
    { label: t.checkout.zipCode,  name: 'zipCode' },
  ];
          ...
          <div className="grid lg:grid-cols-3 gap-12">
            ...
            <div className="lg:col-span-2 space-y-8">
              ...
              <div className="bg-card rounded-[2.5rem] p-8 sm:p-10 border border-border shadow-sm space-y-10">
                <div className="flex items-center gap-4 border-b border-border pb-6">
                  ...
                  <h2 className="text-2xl font-black text-foreground">{t.checkout.shippingTitle}</h2>
                </div>
              ...
              <div className="bg-card rounded-[2.5rem] p-8 sm:p-10 border border-border shadow-sm space-y-10">
                <div className="flex items-center justify-between border-b border-border pb-6">
                  <div className="flex items-center gap-4">
                    ...
                    <h2 className="text-2xl font-black text-foreground">{t.checkout.paymentTitle}</h2>
                  </div>
            ...
            <div className="lg:col-span-1">
              <div className="bg-card rounded-[2.5rem] p-8 border border-border shadow-2xl sticky top-28 space-y-8">
                <h2 className="text-2xl font-black text-foreground">{t.checkout.orderTitle}</h2>
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <div className="flex justify-between items-end">
                          <span className="text-[10px] font-black text-muted-foreground uppercase">
                            {t.checkout.qty}: {item.cartQuantity}
                          </span>
                ...
                <div className="space-y-4 pt-4 border-t border-border">
                  <div className="flex justify-between items-center text-muted-foreground font-bold">
                    <span className="text-sm uppercase tracking-widest italic">{t.checkout.subtotal}</span>
                    <span className="text-foreground">{formatSum(subtotal)}</span>
                  </div>
                  <div className="flex justify-between items-center text-muted-foreground font-bold">
                    <span className="text-sm uppercase tracking-widest italic">{t.checkout.shipping}</span>
                    <span className={shipping === 0 ? 'text-emerald-500 font-black' : 'text-foreground'}>
                      {shipping === 0 ? t.checkout.free : formatSum(shipping)}
                    </span>
                  </div>
                  <div className="flex justify-between items-center text-muted-foreground font-bold">
                    <span className="text-sm uppercase tracking-widest italic">{t.checkout.tax}</span>
                ...
                <div className="bg-muted/30 rounded-2xl p-6 border border-border/50">
                  <div className="flex justify-between items-center">
                    <span className="text-lg font-black text-foreground uppercase tracking-widest">

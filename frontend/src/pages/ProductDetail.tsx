...
import { Star, Heart, Share2, Shield, ChevronLeft, Loader2, ExternalLink, Sparkles } from 'lucide-react';
  ...
  useEffect(() => {
    const getProduct = async () => {
      ...
      try {
        ...
        // Find cheapest market index initially
        if (mapped.markets && mapped.markets.length > 0) {
          const sorted = [...mapped.markets].sort((a, b) => a.price - b.price);
          const cheapest = sorted[0];
          const index = mapped.markets.findIndex(m => m.source === cheapest.source && m.price === cheapest.price);
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-background flex flex-col items-center justify-center">
        ...
        <p className="text-muted-foreground font-bold mt-4">Bozor narxlari tahlil qilinmoqda...</p>
      </div>
    );
  }
    ...
    return (
      <div className="min-h-screen bg-background flex items-center justify-center p-4">
        <div className="bg-card rounded-[2.5rem] p-12 text-center shadow-xl border border-border/50 max-w-lg w-full">
          <h2 className="text-3xl font-black text-foreground mb-6">{error || 'Mahsulot topilmadi'}</h2>
          <Link to="/products">
  ...
  return (
    <div className="min-h-screen bg-background selection:bg-primary/20 animate-fade-in relative overflow-hidden">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        {/* Breadcrumb */}
        <div className="mb-8">
          <Link to="/products" className="group flex items-center gap-2 text-muted-foreground hover:text-primary transition-all w-fit font-bold">
        ...
        <div className="grid lg:grid-cols-2 gap-12 lg:gap-20">
          ...
          <div className="space-y-8">
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-6">
                <div className="flex items-center gap-1">
                  ...
                  <span className="ml-2 font-black text-lg">{product.rating}</span>
                </div>
                <div className="h-4 w-px bg-border" />
                <span className="text-muted-foreground font-bold">{product.reviews} ta tahlil natijasi</span>
              </div>
            </div>
            ...
            {/* Marketplace Comparison - Now Selectable */}
            {product.markets && product.markets.length > 0 && (
              <div className="space-y-4">
                <div className="flex items-center justify-between">
                  ...
                  <span className="text-xs font-bold text-muted-foreground uppercase tracking-widest">Solishtiring va tanlang</span>
                </div>
                <div className="grid gap-3">
                  {product.markets
                    .sort((a, b) => a.price - b.price)
                    .map((market, idx) => {
                      const isSelected = selectedMarketIndex === product.markets?.indexOf(market);
                      return (
                        <button 
                          key={idx} 
                          onClick={() => setSelectedMarketIndex(product.markets?.indexOf(market) ?? 0)}
                        ...
                        >
                          <div className="flex items-center gap-4">
                            <div className={`w-12 h-12 rounded-xl flex items-center justify-center font-black uppercase text-xs transition-colors ${
                              isSelected ? 'bg-primary text-white' : 'bg-muted text-muted-foreground'
                            }`}>
                              {market.source.substring(0, 2)}
                            </div>
                            <div>
                              <p className="font-bold text-foreground text-lg">{market.source}</p>
                              {idx === 0 && (
                                <div className="flex items-center gap-1">
                                  <span className="w-1.5 h-1.5 bg-emerald-500 rounded-full animate-pulse" />
                                  <span className="text-[10px] text-emerald-500 font-black uppercase tracking-widest">Eng arzon variant</span>
                                </div>
                              )}
                            </div>
                          </div>
                          <div className="text-right">
                            <p className="font-black text-xl text-foreground mb-1">{formatSum(market.price)}</p>
                            <span className="text-[10px] font-black text-primary uppercase tracking-widest">Do'konni tanlash</span>
                          </div>
                        </button>
                      );
                    })}
            ...
            <div className="space-y-6 pt-6 border-t border-border/50">
              <div className="flex flex-col gap-4">
                 <a 
                   href={selectedMarket?.url.startsWith('/') ? `https://uzum.uz${selectedMarket.url}` : selectedMarket?.url} 
                   target="_blank" 
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-6 pt-8 border-t border-border/50">
              <div className="flex gap-3">
                ...
                <div>
                  <p className="text-xs font-black text-foreground uppercase">Tashqi do'konga havola</p>
                  <p className="text-[10px] text-muted-foreground font-medium">Xavfsiz va shifrlangan</p>
                </div>
              </div>
              <div className="flex gap-3">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-xs font-black text-foreground uppercase">Narx himoyasi</p>
                  <p className="text-[10px] text-muted-foreground font-medium">Har 15 daqiqada yangilanadi</p>
                </div>
              </div>
              <div className="flex gap-3">
                <div className="w-10 h-10 rounded-xl bg-primary/10 flex items-center justify-center shrink-0">
                ...
                <div>
                  <p className="text-xs font-black text-foreground uppercase">Tasdiqlangan sotuvchi</p>
                  <p className="text-[10px] text-muted-foreground font-medium">Faqat ishonchli do'konlar</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-24 grid lg:grid-cols-3 gap-12">
              ...
              <section className="bg-card rounded-[3rem] p-12 border border-border/50 shadow-sm space-y-6">
                ...
                <div className="prose prose-slate max-w-none">
                  <p className="text-lg text-muted-foreground leading-relaxed font-medium">
                    {product.description || "Experience premium quality with this exceptional product. Carefully crafted with attention to detail, it combines functionality with style to meet your everyday needs. Whether you're looking for performance, durability, or aesthetic appeal, this product delivers on all fronts."}
                  </p>
                </div>
              </section>

              <section className="bg-card rounded-[3rem] p-12 border border-border/50 shadow-sm space-y-8">
                 <h2 className="text-3xl font-black text-foreground">Specifications</h2>
                 <div className="grid sm:grid-cols-2 gap-8">
                    <div className="space-y-4">
                      <h3 className="font-bold text-primary uppercase text-sm tracking-widest">Key Features</h3>
                      <ul className="space-y-3">
                        {['Premium quality materials', 'Durable and long-lasting', 'Modern minimalist design', 'Easy to use and maintain'].map((item, i) => (
                    ...
                    <div className="space-y-4">
                      <h3 className="font-bold text-primary uppercase text-sm tracking-widest">What's in the Box</h3>
                      <ul className="space-y-3">
                        {[`1x ${product.name}`, 'User manual', 'Original Premium Packaging', 'Warranty card'].map((item, i) => (
                          <li key={i} className="flex items-center gap-3 text-foreground font-medium">
                            <div className="w-1.5 h-1.5 bg-primary rounded-full" />
                            {item}
                          </li>
           ...
           <div className="space-y-8">
              <div className="bg-card rounded-[3rem] p-8 border border-border/50 shadow-sm">
                <h2 className="text-2xl font-black text-foreground mb-8">Verified Reviews</h2>
                <div className="space-y-10">
                  {[
                    { name: 'John Smith', initials: 'JS', text: 'Excellent product! Exceeded my expectations in terms of quality and performance.', rating: 5 },
                    { name: 'Anna Martinez', initials: 'AM', text: 'Very satisfied. Great value for money and fast delivery.', rating: 5 },
                    { name: 'Robert Kim', initials: 'RK', text: 'Highly recommend! Perfect fit for my needs.', rating: 4 }
                  ].map((review, i) => (
                      ...
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-0.5">
                            {[...Array(5)].map((_, starI) => (
                              <Star key={starI} className={`w-3 h-3 ${starI < review.rating ? 'fill-primary text-primary' : 'text-muted'}`} />
                            ))}
                          </div>
                        </div>
                      </div>
                      <p className="text-sm text-muted-foreground font-medium italic leading-relaxed">"{review.text}"</p>
                    </div>
                  ))}
                </div>
                <Button variant="ghost" className="w-full mt-10 font-bold text-primary hover:bg-primary/10 rounded-2xl">
                  READ ALL REVIEWS
                </Button>
              </div>
           </div>

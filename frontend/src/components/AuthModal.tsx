...
export function AuthModal() {
  ...
  const handleCredentials = async (e: FormEvent) => {
    ...
    if (authMode === 'login') {
      setIsLoading(true);
      try { await login(email, password); } catch { setError("Email yoki parol noto'g'ri"); }
      setIsLoading(false);
    } else {
          ...
          <div className="relative flex items-center gap-3">
            ...
            <div>
              ...
              <h2 className="text-lg font-black text-white leading-tight">
                {authMode === 'login' ? 'Kirish' : step === 'credentials' ? 'Ro\'yxatdan o\'tish' : 'Profilingiz'}
              </h2>
            </div>
          </div>
              ...
              {(['credentials', 'profile'] as Step[]).map((s, i) => (
                <div key={s} className="flex items-center gap-2">
                  <div className={`w-6 h-6 rounded-full flex items-center justify-center text-[10px] font-black transition-all ${
                    step === s ? 'bg-white text-violet-700' :
                    (i === 0 && step === 'profile') ? 'bg-white/30 text-white' : 'bg-white/15 text-white/50'
                  }`}>
                    {i === 0 && step === 'profile' ? <Check className="w-3 h-3" /> : i + 1}
                  </div>
                  {i === 0 && <div className="flex-1 h-0.5 w-8 bg-white/20 rounded-full">
            ...
            <form onSubmit={handleCredentials} className="space-y-3">
              ...
              >
                {isLoading ? (
                  ...
                  <>
                    {authMode === 'login' ? 'Kirish' : 'Davom etish'}
            ...
            <div className="space-y-5">
              ...
              <div className="flex gap-2 pt-1">
                ...
                <button type="button" onClick={handleRegister} disabled={isLoading}
                  className="flex-2 flex-1 flex items-center justify-center gap-2 py-3.5 rounded-xl bg-violet-600 hover:bg-violet-700 text-white font-black text-sm shadow-md shadow-violet-500/25 transition-all active:scale-[0.98] disabled:opacity-60">
                  {isLoading
                    ? <div className="w-4 h-4 border-2 border-white/30 border-t-white rounded-full animate-spin" />

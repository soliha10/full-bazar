          ...
          {sortedMarkets.length > 0 ? (
            <div className="space-y-1.5">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-2 flex items-center justify-between">
                <span>Do'konlar narxi</span>
                <span className="text-[#0062FF]">{sortedMarkets.length} ta</span>
              </p>
                ...
                {sortedMarkets.slice(0, 3).map((m, idx) => (
                  <div
                    ...
                    className={`flex items-center justify-between rounded-xl px-3 py-2 transition-all ${
                      idx === 0 ? 'bg-green-500/5 ring-1 ring-green-500/20' : 'bg-gray-50/50 hover:bg-gray-50'
                    }`}
                  >
                    <div className="flex items-center gap-2 overflow-hidden">
                       <div className={`w-1.5 h-1.5 rounded-full shrink-0 ${idx === 0 ? 'bg-green-500' : 'bg-gray-300'}`} />
                       <span className={`text-[11px] font-bold truncate ${idx === 0 ? 'text-green-700' : 'text-gray-600'}`}>
                         {m.source}
                       </span>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
                       <span className={`text-xs font-black ${idx === 0 ? 'text-green-700' : 'text-gray-900'}`}>
                         {formatSum(m.price)}
                       </span>
                       {m.url && m.url !== '#' && (
                         <a
              ...
              {sortedMarkets.length > 3 && (
                <button 
                  onClick={() => navigate(`/product/${product.id}`)}
                  className="w-full text-center py-1.5 text-[10px] font-black text-[#0062FF] hover:bg-blue-50 rounded-lg transition-colors uppercase tracking-wider mt-1"
                >
                  Barcha {sortedMarkets.length} ta narxni ko'rish
                </button>
              )}
            </div>
          ) : (
